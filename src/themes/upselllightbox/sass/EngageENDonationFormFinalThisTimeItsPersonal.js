  ...
  if (jQuery("#EN__RootElement").length === 0) {
            ...
            if (console) {
              console.log(
                "beforeHide default\n" +
                  ...
                  "\n" +
                  "pageObject = " +
                  jQuery(pageObject).prop("nodeName") +
                  "\n" +
                  "   ID      = " +
                  jQuery(pageObject).attr("id") +
                  "\n" +
                  "   class      = " +
                  jQuery(pageObject).attr("class") +
                  "\n"
              );
            }
            return true;
    ...
    jQuery(document).ready(function() {
      if (jQuery("form .en__submit").length === 0) {
        if (console) {
          console.log(
            "EngageEnDonationForm: Won't run on confirmation or thank you page."
          );
        }
        return false;
      }
      ...
      EngageENDonationForm.form = jQuery(
        EngageENDonationForm.options.formSelector
      ).hide();
      if (EngageENDonationForm.form.length > 0) {
        ...
        EngageENDonationForm.fields = EngageENDonationForm.document
          .find("input")
          .add("select")
          .add("textarea");
        ...
        window.location.search
          ...
          .forEach(function(arg) {
            if (arg) {
              var nv = arg.split("="),
                argName = EngageENDonationForm.options.translateArg(
                  decodeURIComponent(nv[0])
                );
              if (nv[1]) {
                EngageENDonationForm.args[argName] = decodeURIComponent(
                  nv[1].replace(/</g, "")
                );
              }
            }
        ...
        EngageENDonationForm.windowSize = function() {
          jQuery.extend(EngageENDonationForm.options.windowSizes, {
            isMobile:
              window.innerWidth <
              EngageENDonationForm.options.windowSizes.mobileMax,
            isTablet:
              window.innerWidth >
                EngageENDonationForm.options.windowSizes.tabletMin &&
              window.innerWidth <
                EngageENDonationForm.options.windowSizes.tabletMax,
            isDesktop:
              window.innerWidth >
              EngageENDonationForm.options.windowSizes.desktopMin,
            isTall:
              window.innerHeight >
              EngageENDonationForm.options.windowSizes.tallMin,
          });
          EngageENDonationForm.options.afterWindowSize(
            EngageENDonationForm.options
          );
        };
        jQuery(window).resize(
          jQuery.throttle(200, EngageENDonationForm.windowSize)
        );
        ...
        if (console) {
          console.log(
            "EngageENDonationForm: Loading and juggling donation levels."
          );
        }
        EngageENDonationForm.recurPay = EngageENDonationForm.fields.filter(
          '[name="transaction.recurrpay"]'
        );
        EngageENDonationForm.recurRadio = EngageENDonationForm.fields.filter(
          EngageENDonationForm.options.recurRadioSelector
        );
        EngageENDonationForm.oneTimeRadio = EngageENDonationForm.fields.filter(
          EngageENDonationForm.options.oneTimeRadioSelector
        );
        EngageENDonationForm.recurFrequency = EngageENDonationForm.fields.filter(
          '[name="transaction.recurrfreq"]'
        );
        EngageENDonationForm.recurDay = EngageENDonationForm.fields.filter(
          '[name="transaction.recurrday"]'
        );
        EngageENDonationForm.cardFeeCheckbox = EngageENDonationForm.fields.filter(
          EngageENDonationForm.options.cardFeeCheckboxSelector
        );
        if (EngageENDonationForm.options.showLevel.length > 0) {
          EngageENDonationForm.showLevel =
            EngageENDonationForm.options.showLevel;
          jQuery.extend(EngageENDonationForm.showLevel, {
            set: function() {
              var thisDisplay = jQuery(this),
                ...
                amount = currentDonation[EngageENDonationForm.CD_ISCARDFEE]
                  ? currentDonation[EngageENDonationForm.CD_AMOUNT] +
                    currentDonation[EngageENDonationForm.CD_CARDFEEAMOUNT]
                  : currentDonation[EngageENDonationForm.CD_AMOUNT];
              if (amount % 1 > 0) {
                amount = amount.toFixed(2);
              }
              thisDisplay.text(
                EngageENDonationForm.options.showLevelFormat[frequency].replace(
                  "%%amount%%",
                  amount
                )
              );
        ...
        EngageENDonationForm.donationLevels = EngageENDonationForm.fields.filter(
          '.en__field--donationAmt input[type="radio"]'
        );
        EngageENDonationForm.calcDonation = function() {
          var selectedLevel = EngageENDonationForm.donationLevels.filter(
              ":checked"
            ),
            amount = 0,
            frequency =
              EngageENDonationForm.recurPay.filter(":checked").val() === "Y"
                ? EngageENDonationForm.fields
                    .filter('[name="transaction.recurrfreq"]')
                    .val()
                : "ONE-TIME",
            feeToAdd = 0,
        ...
        EngageENDonationForm.setDonLevel = function(amount, isRecurring) {
          if (
            !EngageENDonationForm.donationLevels
              .filter(
                '[value="' + amount + '"][data-recurring="' + isRecurring + '"]'
              )
              .click().length > 0
          ) {
        ...
        jQuery.extend(EngageENDonationForm.options.afterDonationCallbacks, {
          "00100_ChangeRecurring": function(event) {
            ...
            if (target.attr("name") === "transaction.recurrpay") {
              ...
              EngageENDonationForm.body
                .toggleClass(
                  EngageENDonationForm.options.oneTimeClass,
                  isOneTime
                )
                .toggleClass(
                  EngageENDonationForm.options.recurClass,
                  !isOneTime
                );
        ...
        EngageENDonationForm.fireAfterDonationCallbacks = function(event) {
          ...
          var keys = Object.keys(
            EngageENDonationForm.options.afterDonationCallbacks
          ).sort();
          for (var counter = 0; counter < keys.length; counter++) {
            var thisKey = keys[counter];
            if (
              EngageENDonationForm.options.afterDonationCallbacks.hasOwnProperty(
                thisKey
              ) &&
              typeof EngageENDonationForm.options.afterDonationCallbacks[
                thisKey
              ] === "function"
            ) {
              if (console) {
                console.log(
                  "fireAfterDoantionCallbacks Loop\n" +
                    "thisKey = " +
                    thisKey +
                    "\n"
                );
              }
              EngageENDonationForm.options.afterDonationCallbacks[thisKey](
                event
              );
            }
          }
        };
        EngageENDonationForm.donationLevels.change(
          EngageENDonationForm.fireAfterDonationCallbacks
        );
        EngageENDonationForm.recurRadio.change(
          EngageENDonationForm.fireAfterDonationCallbacks
        );
        EngageENDonationForm.oneTimeRadio.change(
          EngageENDonationForm.fireAfterDonationCallbacks
        );
        EngageENDonationForm.recurFrequency.change(
          EngageENDonationForm.fireAfterDonationCallbacks
        );
        EngageENDonationForm.recurDay.change(
          EngageENDonationForm.fireAfterDonationCallbacks
        );
        EngageENDonationForm.cardFeeCheckbox.change(
          EngageENDonationForm.fireAfterDonationCallbacks
        );
        EngageENDonationForm.otherLevel = {
          radio: EngageENDonationForm.donationLevels.filter(
            '[value="' + EngageENDonationForm.options.otherLevelValue + '"]'
          ),
        ...
        EngageENDonationForm.otherLevel.input
          .change(function(event) {
            if (console) {
              console.log(
                "otherLevel.input.change\n" +
                  "event.currentTarget.id = " +
                  event.currentTarget.id +
                  "\n"
              );
        ...
        if (EngageENDonationForm.options.useAutoNumeric) {
          EngageENDonationForm.otherLevel.input.autoNumeric = new AutoNumeric(
            ".en__field--donationAmt .en__field__input--other",
            EngageENDonationForm.options.autoNumericOptions
          );
        }
        ...
        if (EngageENDonationForm.cardFeeCheckbox.is(":checked")) {
          var donation = EngageENDonationForm.calcDonation(),
            ...
            recurring =
              donation[EngageENDonationForm.CD_FREQUENCY] !== "ONE-TIME";
          EngageENDonationForm.setDonLevel(
            amount / (1 + EngageENDonationForm.options.cardFeeMultiplier),
            recurring
          );
        }
        var donRowBreak = 0;
        ...
        EngageENDonationForm.donationLevels.each(function(index) {
          var thisRadio = jQuery(this),
            ...
            isRecurring = EngageENDonationForm.options.recurLabelRegex.test(
              thisText
            ),
          ...
          if (!EngageENDonationForm.options.showRecurInLabels) {
            thisLabel.text(
              thisText.replace(EngageENDonationForm.options.recurLabelRegex, "")
            );
          }
          if (
            thisRadio.val() !== EngageENDonationForm.options.otherLevelValue
          ) {
        ...
        if (
          EngageENDonationForm.document.find(".en__error").length === 0 &&
          EngageENDonationForm.recurRadio.is(":checked")
        ) {
          if (console) {
            console.log(
              "Initializing: Monthly Checked On First Page Load\n" +
                '".en__error".length = ' +
                EngageENDonationForm.document.find(".en__error").length +
                "\n" +
                ".recurRadio:checked = " +
                EngageENDonationForm.recurRadio.is(":checked") +
                "\n" +
                ".recurDefaultLevel = " +
                EngageENDonationForm.options.recurDefaultLevel +
                "\n"
            );
          }
          EngageENDonationForm.setDonLevel(
        ...
        if (EngageENDonationForm.options.placeHoldersFor.length > 0) {
            ...
            EngageENDonationForm.fields
              ...
              .each(function() {
                var thisField = jQuery(this),
                  thisLabel = EngageENDonationForm.form
                    .find('label[for="' + thisField.attr("id") + '"]')
                    .text();
                thisField.attr("placeholder", thisLabel);
              });
            for (var placeHolderOverridesKey in EngageENDonationForm.options
              .placeHolderOverrides) {
              EngageENDonationForm.fields
                .filter('[type="text"]')
                .filter(placeHolderOverridesKey)
                .attr(
                  "placeholder",
                  EngageENDonationForm.options.placeHolderOverrides[
                    placeHolderOverridesKey
                  ]
                );
            }
          }
          if (EngageENDonationForm.options.placeHoldersFor.includes("select")) {
            EngageENDonationForm.fields.filter("select").each(function() {
              ...
              if (
                EngageENDonationForm.options.hasOwnProperty(
                  "placeHolderOverrides"
                )
              ) {
                for (var placeholderKey in EngageENDonationForm.options
                  .placeHolderOverrides) {
          ...
          if (
            EngageENDonationForm.options.placeHoldersFor.includes("textarea")
          ) {
            EngageENDonationForm.fields.filter("textarea").each(function() {
              ...
              if (thisLabel !== "") {
                thisField
                  .attr("data-placeholder", thisLabel)
                  ...
                  .focus(function() {
                    ...
                    if (
                      textareaField.val() ===
                      textareaField.attr("data-placeholder")
                    ) {
                      textareaField.val("");
                    }
                  })
          ...
          EngageENDonationForm.otherLevel.input.prop(
            "placeholder",
            EngageENDonationForm.options.placeHolderOverrides["other"]
          );
        } else {
          if (console) {
        ...
        if (EngageENDonationForm.options.currentPageNumber > 0) {
          EngageENDonationForm.goPage = function(page, runValidation) {
            ...
            if (
              page > EngageENDonationForm.options.currentPageNumber &&
              validate
            ) {
              if (
                !EngageENDonationForm.parsley.validate({
                  group:
                    EngageENDonationForm.options.pageIDPrefix +
                    EngageENDonationForm.options.currentPageNumber,
                })
              ) {
                return;
              }
            ...
            if (
              EngageENDonationForm.currentPageObject.find(".en__field__error")
                .length > 0
            ) {
              return;
            }
            if (
              EngageENDonationForm.options.pageCallbacks.hasOwnProperty(
                EngageENDonationForm.options.currentPageNumber
              ) &&
              EngageENDonationForm.options.pageCallbacks[
                EngageENDonationForm.options.currentPageNumber
              ].hasOwnProperty("beforeHide")
            ) {
              if (
                !EngageENDonationForm.options.pageCallbacks[
                  EngageENDonationForm.options.currentPageNumber
                  ...
                  EngageENDonationForm.options.currentPageNumber,
                  EngageENDonationForm.currentPageObject
                )
              ) {
                return;
              }
            ...
            if (
              EngageENDonationForm.options.pageCallbacks.hasOwnProperty(page) &&
              EngageENDonationForm.options.pageCallbacks[page].hasOwnProperty(
                "beforeShow"
              )
            ) {
              if (
                !EngageENDonationForm.options.pageCallbacks[page].beforeShow(
                  page,
                  EngageENDonationForm.body.find(
                    "section." +
                      EngageENDonationForm.options.pageIDPrefix +
                      page
                  )
                ...
                EngageENDonationForm.goPage(
                  EngageENDonationForm.options.currentPageNumber + 1,
                  false
                );
                return false;
              }
            ...
            EngageENDonationForm.body
              .removeClass("step1 step2 step3 step4 step5 step6 step7 step8")
              .addClass(
                "step" + EngageENDonationForm.options.currentPageNumber
              );
            EngageENDonationForm.currentPageObject = EngageENDonationForm.body.find(
              "section." +
                EngageENDonationForm.options.pageIDPrefix +
                EngageENDonationForm.options.currentPageNumber
            );
            EngageENDonationForm.currentPageObject
              ...
              .remove();
            EngageENDonationForm.options.breadcrumbs
              .find("[data-step]")
              .each(function() {
                var thisCrumb = jQuery(this);
                thisCrumb.removeClass("current past");
                if (
                  +thisCrumb.attr("data-step") <
                  EngageENDonationForm.options.currentPageNumber
                ) {
            ...
            if (EngageENDonationForm.options.windowSizes.isMobile) {
              if (EngageENDonationForm.options.breadcrumbs.length > 0) {
                jQuery("html, body").animate(
                  {
                    scrollTop: EngageENDonationForm.options.breadcrumbs.offset()
                      .top,
                  },
                  500
                );
          ...
          EngageENDonationForm.nextPage = function(event) {
            ...
            if (
              EngageENDonationForm.options.currentPageNumber <
              EngageENDonationForm.options.lastPageNumber
            ) {
              EngageENDonationForm.options.newPageNumber =
                EngageENDonationForm.options.currentPageNumber + 1;
            }
          };
          ...
          EngageENDonationForm.options.lastPageNumber = jQuery(
            EngageENDonationForm.options.pageStartSelector
          ).each(function(index) {
            var thisItem = jQuery(this),
              container = thisItem.closest(
                EngageENDonationForm.options.pageItemParentSelector
              ),
              thisPage = jQuery("<section/>")
                .addClass(EngageENDonationForm.options.pageItemClass)
                .addClass(
                  EngageENDonationForm.options.pageIDPrefix + (index + 1)
                )
            ...
            if (EngageENDonationForm.options.breadcrumbs.length > 0) {
              ...
              if (
                EngageENDonationForm.options.windowSizes.isMobile &&
                EngageENDonationForm.options.breadcrumbItemMobile.length > 0
              ) {
                thisCrumb = EngageENDonationForm.options.breadcrumbItemMobile.clone();
              } else if (
                EngageENDonationForm.options.windowSizes.isTablet &&
                EngageENDonationForm.options.breadcrumbItemTablet.length > 0
              ) {
                thisCrumb = EngageENDonationForm.options.breadcrumbItemTablet.clone();
              } else if (
                EngageENDonationForm.options.windowSizes.isDesktop &&
                EngageENDonationForm.options.breadcrumbItemDesktop.length > 0
              ) {
              ...
              if (
                index === 0 &&
                EngageENDonationForm.options.showLevel.length > 0
              ) {
            ...
            if (EngageENDonationForm.options.pageButtons.length > 0) {
              if (index === 0) {
                thisPage.append(
                  EngageENDonationForm.options.pageButtons
                    .clone()
                    .append(
                      EngageENDonationForm.options.pageButtonColumn.clone()
                    )
                    .append(
                      EngageENDonationForm.options.continueButton
                        .clone()
                        .wrap(
                          EngageENDonationForm.options.pageButtonColumn.clone()
                        )
                ...
                thisPage.append(
                  EngageENDonationForm.options.pageButtons
                    .clone()
                    .append(thisPage.find(".en__submit"))
                    .append(
                      EngageENDonationForm.options.backButton
                        .clone()
                        .wrap(
                          EngageENDonationForm.options.pageButtonColumn.clone()
                        )
          ...
          jQuery("." + EngageENDonationForm.options.pageItemClass).wrapAll(
            EngageENDonationForm.options.pageBlock.attr("id", "pages")
          );
          if (EngageENDonationForm.options.breadcrumbs.length > 0) {
            EngageENDonationForm.document
              .find("#pages")
              .prepend(
                EngageENDonationForm.options.breadcrumbs.attr(
                  "id",
                  "breadcrumbs"
                )
              );
          ...
          if (console) {
            console.log(
              "EngageENDonationForm: Multi-page structure not loaded."
            );
          }
        ...
        if (EngageENDonationForm.options.useParsley) {
          ...
          EngageENDonationForm.setParsley = function() {
            ...
            if (
              !EngageENDonationForm.options.excludeParsley.includes(thisID) ||
          ...
          EngageENDonationForm.loadParsleyValidators = function() {
            if (EngageENDonationForm.options.useParsley) {
              if (EngageENDonationForm.options.useCreditCardValidator) {
                EngageENDonationForm.cardTypeSelect = function(result) {
                  ...
                  if (result.valid) {
                    EngageENDonationForm.fields
                      .filter(
                        '[name="transaction.paymenttype"][value="' +
                          EngageENDonationForm.options.translateCardTypes[
                    ...
                    EngageENDonationForm.fields
                      .filter('[name="transaction.paymenttype"]')
                      .prop("checked", false);
                  }
                  return result.valid;
                };
                  ...
                  validateString: function(
                    ...
                    if (result) {
                      ...
                      if (EngageENDonationForm.options.useCardTypeSelect) {
                        result = jQuery(
                          parsleyInstance.element
                        ).validateCreditCard(
                          EngageENDonationForm.cardTypeSelect
                        );
                      } else {
                        result = jQuery(
                          parsleyInstance.element
                ...
                EngageENDonationForm.fields
                  .filter("#en__field_transaction_ccnumber")
                  .attr("data-parsley-creditcard", "required")
                  .attr(
                    "data-parsley-errors-container",
                    "#en__field_transaction_ccnumber-errors"
                  );
                EngageENDonationForm.document
                  .find(".en__field--ccnumber > label")
                  .attr("id", "en__field_transaction_ccnumber-errors")
                ...
                if (EngageENDonationForm.options.useCardTypeSelect) {
                  EngageENDonationForm.fields
                    ...
                    .validateCreditCard(
                      EngageENDonationForm.cardTypeSelect,
                      EngageENDonationForm.options.acceptCardTypes
                    );
                }
              }
              window.Parsley.addValidator("expiration", {
                validateString: function() {
                  var cardExpMoField = EngageENDonationForm.fields.filter(
                    '[name="transaction.ccexpire"]#en__field_transaction_ccexpire'
                  );
                  if (cardExpMoField.is(":visible")) {
                    var cardExpMo = cardExpMoField.val(),
                      cardExpYr = EngageENDonationForm.fields
                        .filter(
              ...
              EngageENDonationForm.fields
                ...
                .attr("data-parsley-expiration", "")
                .attr(
                  "data-parsley-errors-container",
                  "#en__field_transaction_ccexpire-errors"
                );
              EngageENDonationForm.fields
                .filter(
                  '[name="transaction.ccexpire"]:not(#en__field_transaction_ccexpire)'
              ...
              EngageENDonationForm.document
                .find('label[for="en__field_transaction_ccexpire"]')
                .before(
                  '<div id="en__field_transaction_ccexpire-errors" class="parsley-errors-list"></div>'
                );
              window.Parsley.addValidator("otherlevel", {
                validateString: function(value, requirement) {
                  if (EngageENDonationForm.otherLevel.radio.prop("checked")) {
                    return (
                      EngageENDonationForm.otherLevel.input.get() >=
                      +requirement
                    );
                  } else {
                    return true;
                ...
                messages: {
                  en:
                    ...
                    EngageENDonationForm.options.otherMinimum +
                    ".",
                },
              });
              EngageENDonationForm.otherLevel.input
                .attr(
                  "data-parsley-otherlevel",
                  EngageENDonationForm.options.otherMinimum
                )
                .attr(
                  "data-parsley-errors-container",
              ...
              EngageENDonationForm.otherLevel.radio
                .parents(".en__field__item")
                .before(
                  '<div id="' +
                    EngageENDonationForm.otherLevel.radio.attr("id") +
                    '-errors" class="parsley-errors-list"></div>'
              ...
              window.Parsley.addValidator("textarea", {
                validateString: function(value, requirement, parsleyInstance) {
                  if (
                    typeof jQuery(parsleyInstance.element).attr(
                      "data-parsley-textarea-required"
                    ) !== "undefined" &&
                    jQuery(parsleyInstance.element)
                      .attr("data-parsley-textarea-required")
                      .toString()
                      .toLowerCase() === "true"
                  ) {
          ...
          fieldRequired
            .siblings('input[type="text"]:not([id^="level"])')
            .each(EngageENDonationForm.setParsley);
          fieldRequired
            .siblings("select")
            .each(EngageENDonationForm.setParsley);
          if (
        ...
        EngageENDonationForm.payTypeSwitch = jQuery(
          EngageENDonationForm.options.payTypeSwitchShow.selector
        );
        if (EngageENDonationForm.payTypeSwitch.length > 0) {
          EngageENDonationForm.payTypeSwitch.change(function() {
            var frequency = EngageENDonationForm.calcDonation()[
                EngageENDonationForm.CD_FREQUENCY
              ],
              payTypeSwitch = jQuery(this);
            if (payTypeSwitch.filter(":checked").val() === "ACH") {
              EngageENDonationForm.fields
                .filter('[name="transaction.paymenttype"][value="ACH"]')
                .prop("checked", true);
            }
            for (var payTypeKey in EngageENDonationForm.options
              .payTypeSwitchTypes) {
              if (
                EngageENDonationForm.options.payTypeSwitchTypes.hasOwnProperty(
                  payTypeKey
                )
              ) {
                for (var payTypesSelKey in EngageENDonationForm.options
                  .payTypeSwitchTypes[payTypeKey]) {
                  if (
                    EngageENDonationForm.options.payTypeSwitchTypes[
                      payTypeKey
                    ].hasOwnProperty(payTypesSelKey)
                  ) {
                    EngageENDonationForm.document
                      .find(
                        EngageENDonationForm.options.payTypeSwitchTypes[
            ...
            if (
              ...
              for (var payTypeValueKey in EngageENDonationForm.options
                .payTypeSwitchTypes[thisPayTypeValue]) {
                if (
                  EngageENDonationForm.options.payTypeSwitchTypes[
                    thisPayTypeValue
                  ].hasOwnProperty(payTypeValueKey)
                ) {
                  var objectSelector =
                    EngageENDonationForm.options.payTypeSwitchTypes[
                      thisPayTypeValue
          ...
          jQuery.extend(EngageENDonationForm.options.afterDonationCallbacks, {
            "00200_SwapPaymentTypes": function(event) {
              ...
              if (
                EngageENDonationForm.options.payTypeSwitchShow.frequencies.includes(
                  EngageENDonationForm.calcDonation()[
                    EngageENDonationForm.CD_FREQUENCY
                  ]
                )
              ) {
                ...
                if (
                  event.currentTarget.id ===
                  EngageENDonationForm.recurRadio.prop("id")
                ) {
                  EngageENDonationForm.payTypeSwitch
                    .filter('[value="ACH"]')
                    .click();
                }
              } else {
        ...
        EngageENDonationForm.form.show().submit(function(event) {
          ...
          if (
            EngageENDonationForm.finished ||
            EngageENDonationForm.options.currentPageNumber === 0
          ) {
            if (!EngageENDonationForm.cardFeeAdded) {
              ...
              if (thisDonation[EngageENDonationForm.CD_ISCARDFEE]) {
                EngageENDonationForm.otherLevel.input.set(
                  thisDonation[EngageENDonationForm.CD_AMOUNT] +
                    thisDonation[EngageENDonationForm.CD_CARDFEEAMOUNT]
                );
              }
            ...
            setTimeout(function() {
              ...
              EngageENDonationForm.currentPageObject
                .siblings("section")
                .removeClass(EngageENDonationForm.options.sectionErrorClass)
                .find(".en__field__error")
                .remove();
              if (
                EngageENDonationForm.currentPageObject.find(".en__field__error")
                ...
                EngageENDonationForm.currentPageObject.addClass(
                  EngageENDonationForm.options.sectionErrorClass
                );
                EngageENDonationForm.form.addClass(
                  EngageENDonationForm.options.formErrorClass
                );
              } else {
                EngageENDonationForm.form.removeClass(
                  EngageENDonationForm.options.formErrorClass
                );
                if (
                  EngageENDonationForm.options.currentPageNumber ===
                  ...
                  if (
                    typeof foursiteModal === "undefined" ||
                    foursiteModal.openUpsell()
                  ) {
